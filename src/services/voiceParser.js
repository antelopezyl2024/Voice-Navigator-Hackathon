...
export function parseVoiceCommand(text) {
  ...
  if ((lower.includes('co2') || lower.includes('carbon')) && lower.includes('descri')) {
    return { command: COMMANDS.DESCRIBE_CO2, raw: text };
  }
  if (lower.includes('co2') || lower.includes('carbon') || lower.includes('emission')) {
    return { command: COMMANDS.SHOW_CO2, raw: text };
  }
  if (lower.includes('agri') || lower.includes('agricultural') || lower.includes('land')) {
    return { command: COMMANDS.SHOW_AGRI, raw: text };
  }
  if (lower.includes('dow') || lower.includes('stock')) {
